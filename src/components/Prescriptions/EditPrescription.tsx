...
import type { PrescriptionPublic, PrescriptionCreate } from "@/client/PrescriptionsService"
...
const formSchema = z.object({
  prescription_type: z.enum(["acute", "chronic", "constitutional", "intercurrent", "nosode", "sarcode", "tautode"]).optional(),
...
const EditPrescription = ({ prescription, onSuccess }: EditPrescriptionProps) => {
    ...
    defaultValues: {
      ...
      medicines: prescription.medicines.length > 0
        ? prescription.medicines.map(m => ({
            ...
            stock_id: m.stock_used_id,
            quantity: m.quantity_used,
          }))
        : [{ medicine_id: "", stock_id: "", quantity: 1 }],
    },
  })
  ...
  const mutation = useMutation({
    mutationFn: (data: PrescriptionCreate) =>
      PrescriptionsService.updatePrescription({ 
        ...
        requestBody: { ...data, case_id: prescription.case_id }
      }),
    onSuccess: () => {
  ...
  const onSubmit = (data: FormData) => {
    const prescriptionData: PrescriptionCreate = {
      ...
      prescription_type: data.prescription_type || prescription.prescription_type,
